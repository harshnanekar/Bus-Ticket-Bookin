...
<html lang="en">
  ...
  <script>
    function renderTable(data) {
      ...
      if (
        ...
        data.content.forEach((item, index) => {
          ...
          row.setAttribute("data-type-id", item.id);
          ...
          cell2.textContent = item.bus_type;
    ...
    document.addEventListener("DOMContentLoaded", async () => {
      const url = "/bus-types-pagination";
      const page = 0;
      const size = 5;
      const search = null;
    ...
    document.getElementById("add-bus-type").addEventListener("click", () => {
      location.href = "/add-bus-type";
    });

    document.addEventListener("click", async (event) => {
      if (event.target.closest(".delete-btn")) {
        const tableRow = event.target.closest("tr");
        const id = tableRow.getAttribute("data-type-id");

        const htmlContent = ` <h3 class="text-base font-semibold text-gray-900" id="modal-title">Delete Bus Type</h3>
                            <div class="mt-2">
                              <p class="text-sm text-gray-500">Are you sure you want to delete bus type ?</p>
                            </div>`;

        document.getElementById("modal-content-delete").innerHTML = htmlContent;
        document
          .getElementById("delete-submit-btn")
          .setAttribute("data-type-id", id);
        document.querySelector(".delete-modal").classList.remove("hidden");
      }

      if (event.target.closest(".update-btn")) {
        const tableRow = event.target.closest("tr");
        let id = tableRow.getAttribute("data-type-id");

        const url = "/fetch-bus-type/" + id;
        const method = "GET";
        ...
        if (data) {
          ...
          htmlContent +=
            '<input id="bus-type-update" class="lms-input" type="text" placeholder="" value="' +
            data.bus_type +
            '">' +
            '<label for="bus-type-update" class="lms-placeholder">Enter Bus Type<span class="text-danger">*</span></label>' +
            "</div>" +
            "</div>";

          document.querySelector(".update-modal-title").innerText = "Bus Type";
          document.getElementById("modal-content-update").innerHTML =
            htmlContent;
          document
            .getElementById("update-submit-btn")
            .setAttribute("data-type-id", data.id);
        }
      }
    });
    ...
    document
      ...
      .addEventListener("click", async (event) => {
        const id = event.target.getAttribute("data-type-id");

        const url = "/delete-bus-type/" + id;
        const method = "GET";
       ...
       //Function TO Update Bus Type
    document.getElementById("update-submit-btn").addEventListener("click",async (event) => {
        const id = event.target.getAttribute("data-type-id");
        const bus_type = document.getElementById("bus-type-update").value.trim();
        console.log("bus type edit ",id,bus_type); 
        ...
        const url = '/update-bus-type';
        const method = "POST";
        const obj = {id,bus_type};
